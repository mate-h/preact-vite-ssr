...
import { deepClone, recordsDiff } from './utils'
...
export type Events = {
  ...
  '@set': { path: StateKeys; value: Record<string, any> }
...
export function save(paths: StateKeys[], config?: Config) {
  ...
  let module: Module = (store) => {
    ...
    function getConfig(path: StateKeys) {
      const { collection = path, realtime = true } = config?.[path] || {}
      return { collection, realtime }
    }

    store.on('@init', (init) => {
      const userId = init.user?.uid || init.clientId
      ...
      store.on('@dispatch', async (state, [event, data]) => {
        if (event === '@set' || event === '@save') {
          return;
        }
        function comparePaths() {
          type Item = ReturnType<typeof recordsDiff>[0] & { path: StateKeys }
          return paths.reduce((acc, path) => {
            ...
            if (initValue && typeof initValue === 'object') {
              ...
              const diffs = recordsDiff(initValue, newValue)
              if (diffs.length >= 1) {
        ...
        const documentRefs = comparePaths().map((diff) => {
          ...
          return {
            ...
            data: diff.type === "update" ? diff.diff : diff.current as any,
        ...
        if (documentRefs.length >= 1 && userId) {
          ...
          documentRefs.forEach((d) => {
            if (d.operation === 'set' || d.operation === 'update') {
              initStates = set(initStates, [d.path, d.id], deepClone(d.current))
            } else if (d.operation === 'delete') {
              const s = deepClone(initStates[d.path])
      ...
      // passive save event local update
      store.on('@save', (state, event) => {
        // update state
        // store.dispatch('@set', event);
      })
      ...
      store.on('@set', (state, event) => {
        ...
        // update in state
        state = set(state, event.path, JSON.parse(clonedValue))
        return set(state, 'savedStates', savedStates)
      });
